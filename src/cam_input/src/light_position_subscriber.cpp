...
{
public:
    LightPositionSubscriber() : Node("light_position_subscriber")
    {
        //create subscribers for the sent light positions
        left_light_subscriber_ = create_subscription<geometry_msgs::msg::PointStamped>("left_light_position", 10, 
        std::bind(&LightPositionSubscriber::left_light_callback, this, std::placeholders::_1));

        right_light_subscriber_ = create_subscription<geometry_msgs::msg::PointStamped>("right_light_position", 10, 
...
private:
    ...
    rclcpp::Subscription<geometry_msgs::msg::PointStamped>::SharedPtr left_light_subscriber_;
    rclcpp::Subscription<geometry_msgs::msg::PointStamped>::SharedPtr right_light_subscriber_;
    int frame_width_;
    int frame_height_;
    ...
    void left_light_callback(const geometry_msgs::msg::PointStamped::SharedPtr msg){
        //scale the possition so that it mathces the display window size
        cv::Point left_point(static_cast<int>(msg->point.x*frame_width_/2), static_cast<int>(msg->point.y*frame_height_));
    ...
    void right_light_callback(const geometry_msgs::msg::PointStamped::SharedPtr msg){
        //works the same as for the left light but the 0.5 of width is added so that the point is displayed on a right hand size
        cv::Point right_point(static_cast<int>(msg->point.x*frame_width_/2 + frame_width_/2), static_cast<int>(msg->point.y*frame_height_));
